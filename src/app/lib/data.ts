...
export const getWorkshops = async (): Promise<CategoryData> => {
  const getWorkshops = gql`
    query WorkshopsCollection {
      collection(handle: "workshops") {
        handle
        products(first: 250) {
          edges {
            node {
  ...
  try {
    return await graphQLClient.request(getWorkshops);
  } catch (error) {
    throw new Error(`
    ${error}`);
  }
